...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b border-yellow-600/30 shadow-soft transition-all duration-300 ${
            ...
            <div className="flex items-center space-x-1">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 hover:bg-yellow-600/20 ${
                    activeSection === item.href.slice(1)
                      ? 'bg-yellow-600/15'
                      : ''
                  }`}
                  style={{ color: activeSection === item.href.slice(1) ? 'hsl(45, 100%, 51%)' : 'hsl(45, 100%, 70%)' }}
                >
                  {item.name}
                </button>
              ))}
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 backdrop-blur-md border-b border-yellow-600/30" style={{ backgroundColor: '#000080' }}>
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className={`block w-full text-left px-3 py-2 text-base font-medium rounded-md transition-all duration-300 hover:bg-yellow-600/20 ${
                  activeSection === item.href.slice(1)
                    ? 'bg-yellow-600/15'
                    : ''
                }`}
